...
def check_system_resources():
    ...
    if available_memory < 8:
        print("⚠️  Low memory detected - using conservative batch size")
...
def main():
    ...
    print("🖼️  Optimized for 80x80 resolution (better image quality)")
    print()
    ...
    try:
        ...
        print(f"\n🔧 OPTIMAL TRAINING CONFIGURATION (80x80):")
        print(f"📱 Device: {device}")
        print(f"🖼️  Image size: {config['img_size']}x{config['img_size']} (Quality optimized)")
        print(f"📦 Batch size: {config['batch_size']} (Optimized for 80x80)")
